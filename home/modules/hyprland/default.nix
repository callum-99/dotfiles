{ lib, config, stripTrailingZeros, machine, keyboardLayout, keyboardVariant, ... }:
let
  inherit (lib) mkEnableOption mkIf;

  cfg = config.module.hyprland;

  enabledMonitors = lib.filterAttrs (name: m: m.enabled) machine.monitors;
  ...
  monitorConfigs = lib.mapAttrsToList (monitorName: m:
    let
      ...
      scale = if m ? scale then stripTrailingZeros m.scale else "auto";
  ...
  nameToId = lib.mapAttrs (name: ws: ws.id) machine.workspaces;
            ...
            builtins.concatLists (builtins.genList (i:
              let
                ...
                name = lib.findFirst
                  ...
                  null
                  (lib.mapAttrsToList (name: value: { inherit name value; }) machine.workspaces);
